...
def searchalbums(owner_id):
    return dbop('select * from albums where albums.owner_id='+owner_id,True)
#输入用户id，返回该用户相簿中的图片以及图片所对应的顺序
def whohelikes(user_id):
    return dbop('select user_name from likes natural join users where likes.target_id=users.id and likes.self_id='+user_id,True)[0]
#输入用户id，返回所有被该用户关注的用户的名字
def wholikeshim(user_id):
    return dbop('select user_name from likes natural join users where likes.self_id=users.id and likes.target_id='+user_id,True)[0]
#输入用户id，返回所有关注该用户的用户的名字
def searchreplies(reply_id):
    return dbop('select item_name,item_id,user_name,user_id,reply_content,added_date from replies natural join users natural join items where items.item_id=replies.item_id and users.id=replies.user_id and replies.reply_id='+reply_id,True)[0]
...
def searchchat(chat_id):
    return dbop('select * from chat where chat.chat_id='+chat_id,True)[0]
